...
/**
 * This Class takes in a file then return Hash map <String, String> of Sub folders and their size and also total.
 * 
...
public class SizeCalculator {
	//static File file = new File("C:\\Users\\selon\\Documents\\Bete Christian"); //Test 1
	static File file = new File(" ");

	public SizeCalculator(File file) {
		this.file = file;
	}
	...
	public static HashMap<String, String> frontDesk ( ) throws IOException {
		System.out.println("1 Y " + file.getAbsolutePath());
		System.out.println("2 Y File exit " + file.exists());

		HashMap<String, String>subFolders_Values = new HashMap<String, String>();
		...
		if (file.isFile()) {
			...
			String total = "0.000 BYTE ";
			int track = 0;
			try {
				for(String str : subFolderList) {
					//subFolderValues[track] = x(new File (orgFolder+"\\"+str),0 + " BYTE");
					subFolders_Values.put(str, x(new File (orgFolder+"\\"+str),0 + " BYTE"));

					total =getRightUnit (getSum(total, subFolders_Values.get(str)));

					System.out.println(str);
					System.out.println(subFolders_Values.get(str));
			...
			subFolders_Values.put("total",total);

		}

		return subFolders_Values;
